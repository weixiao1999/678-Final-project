...
The dataset comes from the website Kaggle. There are nineteen variables included in the dataset originally. I will select several factors to represent the aspects of economic, social, and culture to have a deep analysis. Because the conditions of each country vary greatly, I will classify each country into five Ocontinent,and then group by ocontinent and developing status, so that we can more intuitively and clearly see the differences between regions and status in the subsequent classification. Because Healthexppercapita, Electricity and Gdppercapita are very large, so I take a log of these items in the following analysis.
...
Let's look at the correlation plot. The correlation coefficients of the majority of variables are below 0.5, indicating that they are not highly correlated and can be put into the model at the same time. However, the correlation between schooling and healthexppercapita are very high, over 0.7.Because I am more interested in expenditure, so I will keep the variable per capita health consumption expenditure in the model.Moreover, the coefficients of tuberculosis and fertility rate are relatively higher, thus I will choose fertility rate to analysis for it is a common perception that the incidence of disease is negatively correlated with population life expectancy. Therefore, I prefer to study the unclear relationship of fertility rate and life expectancy. Therefore, I keep Fertility rate. log(healthexppercaptia), inflation, and log(gdppercapita) four economically as well as socially relevant variables for the study.
...
```
The left plot is residual plot and the right plot is Q-Q plot. From the residual plot we can see that most points depicted are randomly scattered above and below the line with 0 as the horizontal axis,in the range of[-2,2], while several points that deviate more from the zero value. The Q-Q plot illustrates that most plots are on the normal distribution line,so the error follows a normal distribution.
